...
def create_train_parser(base_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument('--split', default='chr22', type=str, help='Which split to run on')
    return parser
...
def run_eval(args: typing.Optional[argparse.Namespace] = None, env=None) -> None:
    if env is not None:
        os.environ = env

    if args is None:
        base_parser = create_base_parser()
        eval_parser = create_train_parser(base_parser)
...
def run_predict(args: typing.Optional[argparse.Namespace] = None, env=None) -> None:
    if env is not None:
        os.environ = env
    ...
    arg_dict = vars(args)
    arg_names = inspect.getfullargspec(training.run_predict).args
    ...
    training.run_predict(**train_args)


def run_motif(args: typing.Optional[argparse.Namespace] = None, env=None) -> None:
    if env is not None:
        os.environ = env
...
def run_predict_distributed(args: typing.Optional[argparse.Namespace] = None) -> None:
    """Runs distributed training via multiprocessing.
    """
    if args is None:
        base_parser = create_base_parser()
    ...
    utils.launch_process_group(
           run_predict, args, args.nproc_per_node, args.nnodes,
           args.node_rank, args.master_addr, args.master_port)


def run_motif_distributed(args: typing.Optional[argparse.Namespace] = None) -> None:
    """Runs distributed training via multiprocessing.
    """
    if args is None:
        base_parser = create_base_parser()
    ...
    utils.launch_process_group(
           run_motif, args, args.nproc_per_node, args.nnodes,
           args.node_rank, args.master_addr, args.master_port)

if __name__ == '__main__':
    #run_motif_distributed()
    #run_predict_distributed()
