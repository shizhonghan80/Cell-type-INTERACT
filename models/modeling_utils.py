...
class ProteinModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        for key in list(state_dict.keys()):
            if model.state_dict()[key].shape != state_dict[key].shape: state_dict.pop(key)
...
def accuracy(logits, labels, ignore_index: int = -100):
    with torch.no_grad():
        ...
        predictions = logits.float() > 0.5
        correct = (predictions == labels) * valid_mask
        return correct.sum().float() / valid_mask.sum().float()


def specificity(logits, labels, ignore_index: int = -100):
    with torch.no_grad():
        valid_mask = (labels == 0)
        #predictions = logits.float().argmax(-1)
        predictions = logits.float() > 0.5
        correct = (predictions == labels) * valid_mask
        if valid_mask.sum().float() > 0:
            return correct.sum().float() / valid_mask.sum().float()
        else:
            return 0

def sensitivity(logits, labels, ignore_index: int = -100):
    with torch.no_grad():
        valid_mask = (labels == 1)
        #predictions = logits.float().argmax(-1)
        predictions = logits.float() > 0.5
...
def precision(logits, labels, ignore_index: int = -100):
    with torch.no_grad():
        valid_mask = (labels == 1)
        #predictions = logits.float().argmax(-1)
        predictions = logits.float() > 0.5
        correct = (predictions == labels) * valid_mask
        if predictions.sum().float() > 0:
            return correct.sum().float() / predictions.sum().float()
        else:
            return 0


def Spearman(logits, labels, ignore_index: int = -100):
    """
    ...
    with torch.no_grad():
        ...
        predictions = predictions.sort(dim=-1)[1]
        labels = labels.sort(dim=-1)[1]
        correlation = _rank_correlation_(predictions.float(), labels.float())
        return correlation


def Correlation(logits, labels, ignore_index: int = -100):
    with torch.no_grad():
        predictions = logits.float()
        targets = labels.float()
        pred_mean = torch.mean(predictions)
        pred_std = torch.std(predictions)
        actual_mean = torch.mean(targets)
        actual_std = torch.std(targets)
        cov = torch.mean(torch.mul(predictions-pred_mean,targets-actual_mean))
        correlation = cov / (pred_std * actual_std)
        return correlation


def Pearson(logits, labels, ignore_index: int = -100):
    with torch.no_grad():
        predictions = logits.float()
        targets = labels.float()
        pred_mean = torch.mean(predictions,dim=1)
        pred_std = torch.std(predictions,dim=1)
        actual_mean = torch.mean(targets,dim=1)
        actual_std = torch.std(targets,dim=1)
        cov = torch.mean(torch.mul(predictions-torch.unsqueeze(pred_mean,-1),targets-torch.unsqueeze(actual_mean,-1)),dim=1)
        correlation = cov / (pred_std * actual_std)
        correlation[torch.isnan(correlation)] = 0
        correlation[torch.isinf(correlation)] = 0
        return correlation
...
class SimpleMLP(nn.Module):
    ...
    def __init__(self,
        ...
        self.main = nn.Sequential(
            ...
            nn.Sigmoid()) #nn.Softmax(dim=1) nn.Sigmoid()
...
class Accuracy(nn.Module):
    ...
    def forward(self, inputs, target):
        return Correlation(inputs, target, self.ignore_index)
        #return Pearson(inputs, target, self.ignore_index)
        #return Spearman(inputs, target, self.ignore_index)
...
class MLMHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.classifier = SimpleMLP(hidden_size*2, 512, task_size, 0.1)

    def forward(self, hidden_states, feature=None, targets=None):
        if feature != None: hidden_states = torch.cat((hidden_states, feature), 1)
        hidden_states = self.classifier(hidden_states)
        hidden_states = torch.squeeze(hidden_states,-1)
        outputs = (hidden_states,)
        if targets is not None: 
            loss_fct = nn.MSELoss()
            masked_lm_loss = loss_fct(hidden_states, targets)

            targets = torch.squeeze(targets, -1)
            valid_targets = targets
            valid_prediction = hidden_states
            acc_fct = Accuracy(ignore_index=self._ignore_index)
            metrics = {'Pearson': torch.mean(acc_fct(valid_prediction, valid_targets)),
                       'Accuracy': Correlation(valid_prediction, valid_targets)}
...
class ValuePredictionHead(nn.Module):
    def __init__(self,
        ...
        self.task_size = task_size
        self._ignore_index = ignore_index
        self.classifier = SimpleMLP(hidden_size*2, 512, 1, 0.1)
    ...
    def forward(self, hidden_states, feature=None, targets=None):
        ...
        outputs = (hidden_states,)
        if targets is not None:
            targets = targets.to(hidden_states.dtype)
            targets = torch.squeeze(targets, -1)
            loss_fct = nn.MSELoss(reduce=False)
            positive_weights = (targets == 1) * 1
            negative_weights = (targets == 0) * 1
            weights = positive_weights + negative_weights
            masked_lm_loss = torch.mean(loss_fct(hidden_states, targets) * weights)
...
class MultiValuePredictionHead(nn.Module):
    def __init__(self,
        ...
        self.task_size = task_size
        self._ignore_index = ignore_index
        self.classifier = SimpleMLP(hidden_size, 512, task_size, 0.1)
    ...
    def forward(self, hidden_states, feature=None, targets=None):
        ...
        if targets is not None:
            ...
            negative_weights = (targets == 0) * 1
            weights = positive_weights + negative_weights
            masked_lm_loss = torch.mean(loss_fct(logits, targets) * weights)
...
class SequenceClassificationHead(nn.Module):
    def __init__(self,
        ...
        self.task_size = task_size
        self._ignore_index = ignore_index
        self.classifier = SimpleMLP(hidden_size*2, 512, 2, 0.1)
